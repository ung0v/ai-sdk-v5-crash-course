...
const fewShotExamples = data.data.slice(
  EVAL_DATA_SIZE,
  EVAL_DATA_SIZE + FEW_SHOT_EXAMPLES_COUNT,
);
  ...
  task: async (input) => {
    const result = await generateText({
      ...
      prompt: `
        ...
        Use formal language in the title, like 'troubleshooting', 'discussion', 'support', 'chat', 'research', etc.
